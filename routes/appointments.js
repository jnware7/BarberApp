...
const appointmentsRoutes = new express.Router()
  ...
  appointmentsRoutes.delete('/delete/:id',function (req,res){
    const {id} = req.params
  ...
  // //GET/ appointments/:id
  appointmentsRoutes.get('/:id/edit', (req,res) => {
    const{id} = req.params
    appointments.viewOneAppointments(id)
    .then( appointments => {
      console.log('appointments::',appointments);
      res.render('edit_appointments', {appointments})})
      .catch(error =>{
        res.json({
  ...
  appointmentsRoutes.post('/update/:id', function(req, res) {
